...
uint64_t volatile bulk_movqInsASM(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        for(int i=0; i<bufSize; i++)
            src[i] = rand()%(255);
        ...
        endTime=rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime))>totalTime), "bulk_movq() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        valueCheck(src, dst, bufSize, "[ASMmovq] ");
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_memcpyC(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        memcpy(dst, src, bufSize);
        endTime=rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime))>totalTime), "bulk_memcpy() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        valueCheck(src, dst, bufSize, "[bulk_memcpyC] ");
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_SSE1movaps(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        endTime=rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime))>totalTime), "bulk_SSE1mov() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        valueCheck(src, dst, bufSize, "[bulk_SSE1movaps] ");
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_SSE2movdqa(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        endTime=rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime))>totalTime), "bulk_SSE2mov() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        valueCheck(src, dst, bufSize, "[bulk_SSE2movdqa] ");
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_SSE4movntdq(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        _mm_sfence();
        endTime=rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime))>totalTime), "bulk_SSE4movnt() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        valueCheck(src, dst, bufSize, "[bulk_SSE4movntdq] ");
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_AVX256(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        endTime=rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime))>totalTime), "bulk_AVX256() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        valueCheck(src, dst, bufSize, "[bulk_AVX256] ");
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_AVX512_32(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        endTime=rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime))>totalTime), "bulk_AVX512_32() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        valueCheck(src, dst, bufSize, "[bulk_AVX512_32] ");
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_AVX512_64(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime))>totalTime), "bulk_AVX512_32() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        valueCheck(src, dst, bufSize, "[bulk_AVX512_64] ");
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_clflush(uint64_t bufSize) {
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        endTime = rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime))>totalTime), "bulk_clflush() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        cpuMFence();
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_clflushopt(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        endTime = rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime)) > totalTime), "bulk_clflushopt() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        cpuMFence();
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_Cmemcmp(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        if(memcmp(src, dst, bufSize) == 0)
            ...
            detailedAssert(false, "single_Cmemcmp() - Values mismatch! Compare failed.");
        endTime = rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime)) > totalTime), "bulk_Cmemcmp() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        cpuMFence();
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_SSE2cmp(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        endTime = rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime)) > totalTime), "bulk_SSE2cmp() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        cpuMFence();
    }
    totalTime/=BULK_TEST_COUNT;
...
uint64_t volatile bulk_SSE4cmp(uint64_t bufSize){
    ...
    for(int j=0; j<BULK_TEST_COUNT; j++)
    {
        ...
        endTime = rdtsc();
        detailedAssert(((uint64_t)(totalTime+(endTime-startTime)) > totalTime), "bulk_SSE4cmp() - PERF METRIC OVERFLOW ERR.");
        totalTime += endTime-startTime;
        cpuMFence();
    }
    totalTime/=BULK_TEST_COUNT;
